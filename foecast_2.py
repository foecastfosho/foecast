...
if st is not None:
    ...
    def _cached_price_deck(
        *,
        ...
        """
        Build a price deck with caching.

        This helper wraps ``forecast_model.build_price_deck`` and gracefully
        handles differences in the function signature across versions of the
        library.  If a custom price deck (uploaded CSV) is supplied, it
        attempts to pass it via the ``custom_df`` argument.  Should that
        argument not exist (older versions used ``uploaded_deck``), it falls
        back to using that keyword instead.  If both fail, it calls the
        builder without a custom deck.

        Parameters
        ----------
        start_date : pd.Timestamp
            First month of the deck.
        months : int
            Number of months to forecast.
        oil_start, gas_start, ngl_start : float
            Starting price levels (index prices; differentials are applied in
            the cash flow calculation).
        oil_mom_growth, gas_mom_growth, ngl_mom_growth : float
            Month‑over‑month growth rates (e.g., 0.01 for 1 % per month).
        custom_df : DataFrame, optional
            Custom price deck provided by the user.

        Returns
        -------
        DataFrame
            A monthly price deck DataFrame with columns ``date``, ``oil``,
            ``gas`` and ``ngl``.
        """
        params = dict(
        ...
        if custom_df is not None:
            ...
            try:
                return build_price_deck(custom_df=custom_df, **params)
            except TypeError:
                # Fall back to older argument name
                try:
                    return build_price_deck(uploaded_deck=custom_df, **params)
                except TypeError:
                    pass  # ignore and fall back to no custom deck
        # Default call without custom deck
        return build_price_deck(**params)
