"""
Oil & Gas Royalty Discounted Cash Flow (DCF) Model – Streamlit App
------------------------------------------------------------------

This Streamlit application implements a discounted cash flow model to evaluate
the economics of purchasing a royalty interest in an oil and gas property.  It
closely mirrors the logic used in the accompanying Excel model created by the
`create_dcf_model.py` script, but provides an interactive web interface for
inputting assumptions, forecasting production, and viewing summary results.

Key features implemented here include:

* **Production forecasting** using a hyperbolic decline curve for each well,
  transitioning to exponential decline once the effective decline rate
  reaches a terminal level.  Users can specify the initial production rates,
  Arps b‑factor, and decline parameters for each well.  Wells may be of
  different types (PDP, PUD, future), each with its own start date and
  spacing assumptions.  For demonstration purposes, the app allows up to
  fifteen wells to be modeled, but this can be expanded easily.

* **Royalty interest inputs** including royalty decimal, net revenue interest
  (NRI), tract and unit size, and whether the interest is burdened or
  unburdened.  These inputs are used to compute the owner’s share of net
  revenue on a well‑by‑well basis.

* **Revenue modeling** with separate price decks for oil, natural gas, and
  natural gas liquids (NGL).  Users can choose between a fixed price deck or
  upload their own market strip via CSV.  Price differentials, severance
  taxes, and ad valorem taxes are applied consistent with Texas royalty law
  (4.6 % oil severance tax and 7.5 % gas severance tax【873808517576874†L310-L323】【951814183832884†L85-L88】; default 1 % ad valorem tax【752494034296951†L475-L491】).

* **Timing and discounting** at monthly granularity.  Discount rates are
  converted to monthly equivalents, and present values (PV10, PV15, or
  user‑defined) are computed along with undiscounted cash flow, cumulative
  cash flow, and payback period.

* **Sensitivity analysis** toggles that allow users to run price scenarios
  (e.g., ±10 %, ±20 %), adjust development timing, modify type curve
  assumptions (EUR, b‑factor), and change discount rates.  When toggled,
  results update immediately.

The app uses Pandas and NumPy for data handling and assumes a maximum
forecast horizon of 360 months (30 years).  If you need a longer horizon,
adjust the `forecast_months` parameter accordingly.

To run the app locally, install Streamlit (`pip install streamlit`) and then
execute:

    streamlit run dcf_model_web_app.py

Note: This script is provided for demonstration and educational purposes.  It
is not intended as financial advice.  Always consult with a qualified
    ...
    with tab_dca:
        ...
        if prod_file is not None:
            try:
                ...
                required_cols = {'date', 'well_id', 'rate'}
                if not required_cols.issubset(df.columns):
                    st.error('CSV must contain columns: date, well_id, rate')
                else:
                    df['date'] = pd.to_datetime(df['date'])
                    # Sort by date
                    df = df.sort_values(['well_id', 'date'])
                    wells_list = df['well_id'].unique().tolist()
                    selected_well = st.selectbox('Select well for analysis', wells_list)
                    well_data = df[df['well_id'] == selected_well].copy()
                    # Convert to monthly time axis (t in months since first production)
                    well_data = well_data.sort_values('date')
                    well_data['t_months'] = (well_data['date'] - well_data['date'].iloc[0]).dt.days / 30.0
                    # Optional smoothing / outlier removal
                    if st.checkbox('Apply simple outlier removal (3σ filter)', value=True):
                        q_series = well_data['rate']
                        mean = q_series.mean()
                        std = q_series.std() if q_series.std() > 0 else 1.0
                        mask = (np.abs(q_series - mean) <= 3 * std)
                        well_data = well_data[mask]
                        if len(well_data) < 3:
                            st.warning('Not enough data points after outlier removal. Please disable the filter or provide more data.')
                    # Decline model selection
                    model_choice = st.selectbox('Decline model', ['Exponential', 'Hyperbolic', 'Harmonic'])
                    b_override = None
                    ...
                    min_dt = st.number_input('Minimum terminal decline (%)', value=5.0, min_value=0.0, max_value=30.0, step=0.5) / 100.0
                    if st.button('Fit decline curve'):
                        t = well_data['t_months'].values
                        q_vals = well_data['rate'].values
                        # Avoid zeros or negative values
                        mask = q_vals > 0
                        t = t[mask]
                        q_vals = q_vals[mask]
                        result = fit_decline_model(t, q_vals, model_choice, b_override=b_override, min_terminal_decline=min_dt)
                        if 'error' in result:
                            st.error(f"Error fitting curve: {result['error']}")
                        else:
                            st.subheader('Fitted Parameters')
                            st.write(f"Initial rate (qᵢ): {result['qi']:.2f}")
                            st.write(f"Initial decline rate (Dᵢ): {result['Di']:.4f} per month")
                            st.write(f"b‑factor: {result['b']:.3f}")
                            st.write(f"R²: {result['R2']:.3f}")
                            st.write(f"RMSE: {result['RMSE']:.3f}")
                            # Plot actual vs fitted
                            fig, ax = plt.subplots()
                            ax.plot(t, q_vals, 'o', label='Observed')
                            ax.plot(t, result['q_pred'], '-', label='Fitted')
                            ax.set_xlabel('Time (months)')
                            ax.set_ylabel('Production rate')
                            ax.set_title(f'Decline Curve Fit – {model_choice}')
                            ax.legend()
                            st.pyplot(fig)
            except Exception as e:
